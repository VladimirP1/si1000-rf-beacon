...
#define SAMPLE_RATE       100            // ADC conversion rate in Hz
//-----------------------------------------------------------------------------
// Function PROTOTYPES
//-----------------------------------------------------------------------------
...
void PORT_Init (void)
{
   ...
   P5MDIN  &= ~0x03;                      // Set P5.0 as an analog input
   P5 |= 0x03;


   SFRPAGE = ADC0_PAGE;                   // Enable P5.0 as input into AMUX0
   AMUX0P5 |= AMUX0P5_B0__CONNECTED;
   SFRPAGE = CONFIG_PAGE;

   // Power Potentiometer from P0.0
   P0MDOUT |= 0x01;                       // Set pin to push-pull
   P0MDIN  |= 0x01;                       // Set to digital
   P0 |= 0x01;                            // Set High
...
// Parameters:    None
//
// Configure Timer2 to 16-bit auto-reload and generate an interrupt at <SAMPLE_RATE>
...
void TIMER2_Init (void)
{
   ...
   TMR2RL = 65535 - (SYSCLK / 12 / SAMPLE_RATE);   // Init reload value
...
// Parameters:    None
//
// Configures ADC0 to make single-ended analog measurements on pin P5.0
...
void ADC0_Init (void)
{
   ...
   ADC0CN |= ADC0CN_ADBMEN__BURST_ENABLED;// Enable Burst Mode


   ADC0AC = ADC0AC_ADRPT__ACC_16 |        // Right-justify results, shifted right
            ADC0AC_ADSJST__RIGHT_SHIFT_2; // by 2 bits. Accumulate 16 samples for
   ...
   //ADC0CN_ADEN = 1;                     // Don't Enable ADC0
