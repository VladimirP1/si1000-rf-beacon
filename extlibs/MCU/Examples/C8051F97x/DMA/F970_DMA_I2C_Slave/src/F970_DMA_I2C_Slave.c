...
INTERRUPT_PROTO (DMA_FULL_LENGTH_ISR, DMA0FULL_IRQn);
INTERRUPT_PROTO (TIMER3_ISR, TIMER3_IRQn);
INTERRUPT_PROTO (I2CSlave_ISR, I2C0_IRQn);
...
void main (void)
{
	...
	I2CSlave_Init ();                      // Configure and enable SMBus
	DMA_Init();

	EIP2 |= 0x80;           // Enable I2C interrupt high priority
	EIE2 |= 0x80;           // Enable I2C interrupt
...
void DMA_Init (void)
{
	...
	DMA0NCF  = DMA0NCF_PERIPH__I2C_DATA_TO_XRAM;  // Select XRAM to I2C peripheral
	DMA0NCF |= DMA0NCF_IEN__ENABLED;              // Enable DMA full-length interrupt
	...
	DMA0INT &= ~DMA0INT_CH1I__BMASK;              // Clear interrupt bit for channel 1
	DMA0NCF  = DMA0NCF_PERIPH__XRAM_TO_I2C_DATA;  // Select I2C peripheral to XRAM
	DMA0NCF |= DMA0NCF_IEN__ENABLED;              // Enable DMA full-length interrupt
	DMA0NCF |= (LSB << 4);                        // Use LSB to spec endian bit for compiler independence
	...
	EIE1    |= EIE1_EDMA0__ENABLED;               // Enable DMA interrupt
}
...
//
//-----------------------------------------------------------------------------
INTERRUPT(I2CSlave_ISR, I2C0_IRQn)
{
	SFRPAGE = I2C0_PAGE;
...
//
//-----------------------------------------------------------------------------
INTERRUPT(DMA_FULL_LENGTH_ISR, DMA0FULL_IRQn)
{
	SFRPAGE = I2C0_PAGE;
