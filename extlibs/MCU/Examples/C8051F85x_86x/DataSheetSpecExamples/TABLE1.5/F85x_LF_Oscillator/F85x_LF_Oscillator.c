...
// 6) Remove any and all power sources to the board including the debugger.
// 7) Connect a power supply to VDD (TP5) set to 3.6V.
// 8) Use an oscilloscope to monitor P0.0. The low frequency oscillator is
//    routed to this pin for this program.
// 9) Sweep the supply voltage down to 2.2V. During the sweep, the frequency
