...
// 5) Adjust the potentiometer so that the measured voltage at P1.2 on J25 is
...
// 6) Adjust the potentiometer up from GND until DS5 turns on. DS5 will turn on
//    when the ADC reaches the maximum 10 bit value (0x03FF).
// 7) Measure the voltage at P1.2 on J25. The voltage should read between the
...
//    not display as 0 due to a 1k resistor placed between P1.2 and the wiper
//    of the potentiometer.
// 6) Use the switch at P1.7 (S1) to set the internal ADC reference level to
...
//
// Return Value:  None
// Parameters:    U16 ADC0_Sampling_Rate
//
...
{
   int i;                              // For loop variable initialization
