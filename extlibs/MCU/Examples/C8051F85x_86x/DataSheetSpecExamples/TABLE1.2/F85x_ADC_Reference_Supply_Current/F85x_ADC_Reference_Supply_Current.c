...
// 4) Remove all shorting blocks on the board except for the block(s) across
//    the following pins:
...
// 6) Use the switch at P1.7 (S1) to disable the internal reference.
// 7) Measure the current across Imeasure and subtract the value from the
...
//    a) S1 and P1.7 on J27
//    b) S2 and P2.1 on J27
// 5) Use the switch at P2.1 (S2) to set the internal ADC reference to a low
//    power mode.
...
void main (void)
{
   ...
   while (1)
   {
      int i;                           // For loop variable initializer
