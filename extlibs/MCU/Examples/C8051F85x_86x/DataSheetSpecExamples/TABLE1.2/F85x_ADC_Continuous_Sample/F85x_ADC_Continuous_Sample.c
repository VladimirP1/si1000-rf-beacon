...
// 5) Measure the current across Imeasure. This reading is the current with
//    the ADC enabled.
...
//    b) S1 and P1.7 on J27
//    c) S2 and P2.1 on J27
// 5) Press the switch at P1.7 (S1) once. This will initialize the ADC to run
...
//    b) S1 and P1.7 on J27
//    c) S2 and P2.1 on J27
// 5) Press the switch at P1.7 (S1) twice. This will initialize the ADC to run
//    at the High Speed 12 Bit mode.
// 6) Use an oscilloscope to monitor P1.0. A square wave should be seen at
...
//    b) S1 and P1.7 on J27
//    c) S2 and P2.1 on J27
// 5) Press the switch at P1.7 (S1) twice.
// 6) Press the switch at P2.1 (S2) once. This will initialize the ADC to run
...
//    c) S2 and P2.1 on J27
// 5) Press the switch at P1.7 (S1) twice.
// 6) Press the switch at P2.1 (S2) three times. This will initialize the ADC
...
//    c) S2 and P2.1 on J27
// 5) Press the switch at P1.7 (S1) twice.
// 6) Press the switch at P2.1 (S2) four times. This will initialize the ADC
...
{
   // Local variables
   U8 currTestMode = CURRENT_HIGH_POWER;
   U8 nextTestMode = CURRENT_LOW_POWER;
   U8 nextThroughputRate = HIGH_SPEED_10_BIT;
   U16 debounceCounter;
...
//
//-----------------------------------------------------------------------------
void ADC0_Init (U8 powerMode, U8 resolution)
{
