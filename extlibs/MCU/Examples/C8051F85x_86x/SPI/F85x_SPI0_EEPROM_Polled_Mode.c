...
//    - J27: DS5 P1.0 - P1.0
//    - JP2
// 2) Connect the device to a SPI EEPROM.
...
// 5) Run the code. The LED will blink fast during the write/read/verify
//    operations.
...
// 7) Run the code. The terminal will print the progress of the write/read
...
void SPI0_Init(void)
{
   SPI0CFG = 0x40;                     // Master mode, mode 0,0 for
                                       // phase/polarity
   SPI0CN = 0x0C;                      // 4-wire mode

   // The equation for SPI0CKR is (SYSCLK/(2*F_SCK_MAX))-1, which yields
   ...
   SPI0CN |= 0x01;                     // Enable the SPI module
}
