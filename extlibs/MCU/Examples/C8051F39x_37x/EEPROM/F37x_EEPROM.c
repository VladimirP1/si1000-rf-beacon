...
U8 SEG_XDATA SAMPLE_ARRAY[512];        // Test array for EEPROM
...
SBIT (SCL, SFR_P2, 2);                 // SMBus on P2.2
SBIT (SDA, SFR_P2, 3);                 // and P2.3
...
INTERRUPT_PROTO (TIMER3_ISR, INTERRUPT_MY_TIMER);
INTERRUPT_PROTO (SMBUS0_ISR, INTERRUPT_MY_SMBUS);

void EEPROM_ByteWrite(U16 addr, U8 dat);
void EEPROM_WriteArray(U16 dest_addr, U8* src_addr,
                       U8 len);
U8 EEPROM_ByteRead(U16 addr);
void EEPROM_ReadArray(U8* dest_addr, U16 src_addr,
                      U8 len);
...
void main (void)
{
   ...
   U16 j;                              // Temporary counter variable
...
//
//-----------------------------------------------------------------------------
INTERRUPT(SMBUS0_ISR, INTERRUPT_MY_SMBUS)
{
...
// Return Value : None
// Parameters   :
//   1) U16 addr - address to write in the EEPROM
//                        range is: 0 to 512
//
...
// Return Value : None
// Parameters   :
//   1) U16 dest_addr - beginning address to write to in the EEPROM
//                        range is: 0 to 512
//
...
//
//-----------------------------------------------------------------------------
void EEPROM_WriteArray(U16 dest_addr, U8* src_addr, U8 len)
{
   U8 i;
...
//
// Parameters   :
//   1) U16 addr - address to read data from the EEPROM
//                        range is: 0 to 512
//
...
//                        range is full range of character: 0 to 255
//
//   2) U16 src_addr - beginning address to read data from the EEPROM
//                        range is: 0 to 512
//
...
//
//-----------------------------------------------------------------------------
void EEPROM_ReadArray (U8* dest_addr, U16 src_addr, U8 len)
{
   while (SMB_BUSY);                   // Wait for SMBus to be free.
   SMB_BUSY = 1;                       // Claim SMBus (set to busy)
